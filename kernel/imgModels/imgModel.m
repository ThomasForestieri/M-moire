...
omega     = varargin{2};
x         = varargin{3}(:);
if strcmp(method, 'makimaInter')
    T = makimaInterpolation(T, omega, x, optn{:});
    varargout = T;
else
...
%------------------------------------------------------------------------------
function Tc = makimaInterpolation(T, omega, x, varargin)
% Interpolation using Makima
dim = length(omega)/2;
m   = size(T);
...
% Assuming 2D interpolation
[X1, X2] = meshgrid(linspace(omega(1), omega(2), m(1)), ...
                  linspace(omega(3), omega(4), m(2)));
